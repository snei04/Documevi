...
exports.updateCampo = async (req, res) => {
    ...
    try {
        const [result] = await pool.query(
            'UPDATE oficina_campos_personalizados SET nombre_campo = ?, tipo_campo = ?, es_obligatorio = ?, validar_duplicidad = ? WHERE id = ?',
            [nombre_campo, tipo_campo, es_obligatorio || false, validar_duplicidad || false, id]
        );
