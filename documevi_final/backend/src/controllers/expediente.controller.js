...
      // Tiene un préstamo activo: puede ver los documentos
      // ✅ SOLUCIÓN: Se completa la consulta SQL que faltaba.
      const [documentos] = await pool.query(`
        SELECT d.*, ed.orden_foliado, ed.fecha_incorporacion, ed.requiere_firma
        FROM expediente_documentos ed
        JOIN documentos d ON ed.id_documento = d.id
...
exports.createDocumentoFromPlantilla = async (req, res) => {
    ...
    try {
        ...
        // --- INICIO DE CAMBIOS ---
        // 1. Obtenemos también el diseño de la plantilla desde la base de datos.
        ...
        // 2. Parseamos el JSON del diseño. Si está vacío, usamos un objeto por defecto.
        ...
        // --- INICIO DE NUEVA LÓGICA DE DIBUJADO ---
        // 3. Embeber las fuentes que vamos a usar.
        ...
        // 4. Dibujamos el título y otros elementos fijos del diseño (si existen).
        if (diseño.titulo) {
        ...
        // 5. Recorremos los "placeholders" del diseño y los rellenamos con los datos enviados.
        if (diseño.placeholders) {
            for (const placeholder of diseño.placeholders) {
                // Buscamos el valor correspondiente en los datos que llegaron del formulario.
                const valor = datos_rellenados[placeholder.campo];
                
                if (valor) { // Solo dibujamos si el dato fue proporcionado.
                    page.drawText(String(valor), { // Convertimos a String para asegurar que se pueda dibujar
                        x: placeholder.x,
                        y: placeholder.y,
        ...
        // --- FIN DE NUEVA LÓGICA DE DIBUJADO ---
