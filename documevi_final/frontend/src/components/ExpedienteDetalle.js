...
const ExpedienteDetalle = () => {
    ...
    const handleAddDocumento = async (e) => { e.preventDefault(); if (!selectedDocumento) return toast.warn('Por favor, seleccione un documento.'); try { await api.post(`/expedientes/${id}/documentos`, { id_documento: selectedDocumento }); toast.success('Documento añadido con éxito.'); setSelectedDocumento(''); fetchExpediente(); } catch (err) { toast.error(err.response?.data?.msg || 'Error al añadir el documento.'); } };
    ...
    return (
        <div style={{ padding: '20px' }}>
            ...
            <table border="1" style={{ width: '100%', borderCollapse: 'collapse', background: '#fff' }}>
                <thead><tr><th>Foliado</th><th>Radicado</th><th>Asunto</th><th>Fecha Incorporación</th><th>Estado Firma</th><th>Acciones</th></tr></thead>
                <tbody>
                    {expediente.documentos.map(doc => (
                        <tr key={doc.id}>
                            ...
                            <td style={{ display: 'flex', gap: '5px', padding: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                                {!doc.firma_hash && <button onClick={() => openSignatureModal(doc.id)} style={{ backgroundColor: 'green', color: 'white' }}>Firmar</button>}
            ...
            <Modal isOpen={showSignatureModal} onRequestClose={closeSignatureModal} contentLabel="Firmar Documento" style={{ content: { top: '50%', left: '50%', right: 'auto', bottom: 'auto', marginRight: '-50%', transform: 'translate(-50%, -50%)', width: '550px' } }}>
