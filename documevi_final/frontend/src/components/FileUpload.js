import React, { useState, forwardRef } from 'react'; // 1. Importamos 'forwardRef' desde React
import styled from 'styled-components';

// 2. Envolvemos todo el componente en 'forwardRef'
//    Ahora recibe 'props' como primer argumento y 'ref' como segundo.
const FileUpload = forwardRef(({ onFileChange, inputId = 'file' }, ref) => {
  const [text, setText] = useState('Haz clic para subir un archivo');
  ...
  return (
    <StyledWrapper>
      <label className="custum-file-upload" htmlFor={inputId}>
        ...
        {/* 3. Asignamos la 'ref' que recibimos al input real */}
        <input type="file" id={inputId} onChange={handleFileChange} ref={ref} />
      </label>
    </StyledWrapper>
  );
});
